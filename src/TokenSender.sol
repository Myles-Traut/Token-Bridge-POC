...
contract TokenSender is ITokenSender {
    ...
    function bridge(
        ...
        uint256[] memory amounts =
            _swapExactTokensForTokens(_tokenAmountIn, _minAmountOut, path, address(this), block.timestamp + 1000);

        SwapDetails memory details =
            SwapDetails({originalToken: _tokenAddress, minAmountOut: _minAmountOut, recipient: msg.sender});
        ...
        emit MessageSent(messageId, _destinationChainSelector, message.tokenAmounts[0].amount, address(feeToken), fee);
    }
